...
export default function AdminLoanDetailsPage() {
  ...
  useEffect(() => {
    const fetchLoanDetails = async () => {
      try {
        setLoading(true)
        const response = await axios.get(`${API_URL}/admin/loans/${params.id}`)
        ...
        setLoan(mockLoan)
        setSelectedStatus(mockLoan.status)
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleStatusUpdate = async () => {
    if (!selectedStatus || !loan) return

    try {
      await axios.put(`${API_URL}/admin/loans/${loan._id}/status`, {
  ...
  return (
    <DashboardLayout title="Loan Details" allowedRoles={["admin"]}>
            ...
            <TabsContent value="actions">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Change the status of this loan application</CardDescription>
                </CardHeader>
                ...
                <CardFooter>
                  <Button onClick={handleStatusUpdate} disabled={!selectedStatus || selectedStatus === loan.status}>
                    Update Status
                  </Button>
                </CardFooter>
              </Card>
